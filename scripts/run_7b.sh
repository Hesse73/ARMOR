...
project_name='ARMOR'
exp_name="Qwen2.5-Math-7B-restart_DAPO-mix_anchored_${mix_ratio}-only_cor_ref_${ref_n}"

use_ref_model=True
# NOTE: (1st run, DAPO) In our experiments, we first train DAPO on Qwen2.5-Math-7b base model, pick the best-performing ckpt before over-optimization,
# (2nd run, ARMOR) and put the ckpt here for continual training.
# (3rd run, ARMOR + reset) For reference reset, since we were not sure about the optimal reset frequency, we simply resume from 
# the checkpoint of {80, 120, 160}-step model from the 2nd run above (cf. our paper's reset frequencey ablation).
# 
# We recommend you to train more steps for the 2nd run, and then manually pick those best-performing ckpt here to continue from, this normally produces better results
#
# Remember to modify ${exp_name} if you use different models!
...
python3 -m recipe.dapo.main_dapo \
    ...
    algorithm.mix_rollout.main_ratio=${mix_ratio} \
    ...
    actor_rollout_ref.rollout.val_kwargs.n=80 \
