...
ref_policy_sync_steps=0  # no auto reset
project_name='ARMOR'
exp_name="Qwen3-8B-Base-restart-DAPO-mix_anchored_${mix_ratio}-sync_1_til_180-only_cor_ref_${ref_n}"
...
# (2nd run, ARMOR) and put the ckpt into _run_8b.sh for reset@1 training
# (3nd run, ARMOR) At 180 steps of the 2nd-run, we disable the reset@1 mode, and fill the trained ckpt below:
ref_model=${MODEL_TO_CONSUME:-"Put the model ckpt here for continual training with ARMOR"}
MODEL_PATH=${ref_model}
resume_from=${FSDP_MODEL_TO_CONSUME:-"Put the FSDP model directory here for continual training with ARMOR"}
