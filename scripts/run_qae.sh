...
project_name='ARMOR'
exp_name="Qwen2.5-Math-7B-restart_qae_0.45-mix_anchored_${mix_ratio}-only_cor_ref_${ref_n}"

use_ref_model=True
# NOTE: (1st run, QAE) In our experiments, we first train QAE on Qwen2.5-Math-7b base model, pick the best-performing ckpt before over-optimization,
# (2nd run, ARMOR) and put the ckpt here for continual training.
# (3rd run, ARMOR + reset) For reference reset, set your 2nd-run trained model ckpt to ${ref_model}
# 
