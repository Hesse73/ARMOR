...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_rollout(self, trust_remote_code=False):
        ...
        if rollout_name == "hf":
            ...
            if self.role == 'actor_rollout_ref':    # NOTE: kx's special modification
                rollout_sharding_manager_ref = FSDPVLLMShardingManager(
                ...
                log_gpu_memory_usage("After building ref model's sharding manager", logger=logger)
                return rollout, rollout_sharding_manager, rollout_sharding_manager_ref
    ...
    def init_model(self):
        ...
        if self._is_rollout:
            if self.role == "actor_rollout_ref":   # NOTE: kx's special modification
                self.rollout, self.rollout_sharding_manager, self.rollout_sharding_manager_ref = self._build_rollout(
                    trust_remote_code=self.config.model.get("trust_remote_code", False)
                )
            else:
    ...
    @register()
    def _update_mix_ratio(self, ratio:float):
        import dataclasses
        if hasattr(self.actor.config, 'mix_probs_main_ratio'):
            self.actor.config = dataclasses.replace(self.actor.config, mix_probs_main_ratio=ratio)
            print(f"Set mix_probs_main_ratio to {ratio}")

    @register() # default is all to all
    def _sync_actor_to_ref(self):
        assert self._is_ref and self._is_actor
        print("Syncing actor model to reference model...")
        # copy actor model params to ref model
        ...
        with FSDP.state_dict_type(
            self.actor_module_fsdp, StateDictType.FULL_STATE_DICT, save_policy
        ):
            actor_full_state = self.actor_module_fsdp.state_dict()

        with FSDP.state_dict_type(
            self.ref_module_fsdp, StateDictType.FULL_STATE_DICT, save_policy
        ):
            self.ref_module_fsdp.load_state_dict(actor_full_state)
    ...
    def generate_sequences_ref(self, prompts: DataProto):
        # Support all hardwares
        prompts = prompts.to(get_device_id())

        assert self._is_rollout and self._is_ref
        ...
        timing_generate = {}
        with self.rollout_sharding_manager_ref:
        ...
        timing_generate.update(self.rollout_sharding_manager_ref.timing)
